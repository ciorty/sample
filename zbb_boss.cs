...
public class zbb_boss : MonoBehaviour
{
    ...
    public void inert_agent_enabled_false()//miscarea initiala, fara miscare, din care trece in patrol
    {
        if (movement == "inert_agent_enabled_false")
        {
            ...
            if (once_start == 0)
            {
                ...
                no_clones_time = 30 * (snts.enns_number+1);
                once_start = 1;
            }
    ...
    public void deplasare_spre_ego()//movement == "deplasare_spre_ego";
    {
        if (movement == "deplasare_spre_ego")
        {
            mergi();
            //alergi();
            //target = ego_shadow.transform;// GameObject.Find("Capsule").transform;
            ...
            if (snts.proxi == 1)
            {
                ...
                //ego_shadow = GameObject.Find("Capsule").transform;
                //target = ego_shadow;
            ...
            if (snts.proxi == 0)
            {
                ...
                // if (dw == 0)
                // {
                //    GameObject.Find("test_respawn").transform.position = GameObject.Find("Capsule").transform.position;
                //   dw = 1;
                //}
    ...
    public void roteste_corpul_spre_ego()
    {
      ...
      mesh_agent.transform.rotation = Quaternion.RotateTowards(mesh_agent.transform.rotation, Quaternion.LookRotation(dirLookEgo), 3.0f);//cu cat mai mare -> mai rapid
    ...
    public void roteste_corpul_spre_inainte()
    {
       ...
       mesh_agent.transform.rotation = Quaternion.RotateTowards(mesh_agent.transform.rotation, Quaternion.LookRotation(transform.forward), 3.0f);//cu cat mai mare -> mai rapid
    ...
    */
    ...
    void Update()
    {
        if (inert == 1)//LA MOARTEA ENN-ULUI
        {
            ...
            //mesh_agent.transform.position = transform.position;
            //DISPARE ICONUL
            //transform.Find("Iconss").gameObject.SetActive(false);

            //NU MAI ESTE NAV AGENT
            agent.enabled = false;

            //DEVINE KINEMATIC (PRACTIC DUPA CE A CAZUT RAGDOL-UL)
            ...
            if (check_inert == 0)
            {
                ...
                //snts.kills_number = snts.kills_number + 1;
            ...
            //ENN-UL DISPARE
            /*
            foreach (GameObject mats in fadeObject)//obiectele din care este compus enn-ul: corp/pusca/etc
            {
                foreach (Material matt in mats.GetComponent<Renderer>().materials)
                {
                    Color c = matt.color;
                    if (startFade == 0)
                    {
