...
public class enn_civil : MonoBehaviour
{
    ...
    public animations_enn_0 movings;
    ...
    void Start()
    {
        ...
        */
        prefabBullet = (GameObject)Resources.Load("bullet_enn_weapon_1", typeof(GameObject));
        ...
        //to_shoot = GameObject.Find("enn_shoot_dummie").gameObject;
        ...
        resp = new List<float>();
        atack = 2;
        movings = mesh_agent.GetComponent<animations_enn_0>();
    ...
    public void inert_agent_enabled_false(int timing)//miscarea initiala, fara miscare, din care trece in patrol
    {
    ...
    public void heading_patrol_point(int timing)//enn alege si se indreapta catre un punct random de patrulare
    {
        if (movement == "heading_patrol_point")
        {
            ...
            {
                if (aproape == 1 && Vector3.Angle(dirLookEgo, dirMers) <= 45.0f && snts.proxi == 1)
                {
                    stai();
                    agent.speed = 0.0f;
                    setAction = 0;
                ...
                if (aproape == 0 || Vector3.Angle(dirLookEgo, dirMers) > 45.0f || snts.proxi == 0)
                {
                    mergi();
                    agent.speed = 2.0f;
                }
    ...
    public void take_view()// search "view_point"
    {
        ...
        bool hidek = Physics.Linecast(GameObject.Find("Capsule").transform.position, snts.move_array[reg_index_2].transform.position);
    ...
    public void take_hide()// search "hide_point"
    {
        ...
        bool hidek = Physics.Linecast(GameObject.Find("Capsule").transform.position, snts.move_array[reg_index].transform.position);
    ...
    {
        if (Physics.Raycast(transform.Find("enn_shoot_dummie").transform.position, dirLookEgo, out hitShoot, 2000))
        {
            if (hitShoot.collider)
            {
                if (vede_peste_tot == 0)
                {
                    if (hitShoot.collider.name == "Capsule" && snts.proxi == 1)
                    {
                        far = far - 1;
                        if (far <= 0)
                        {
                ...
                if (vede_peste_tot == 1)
                {
                    if (hitShoot.collider.name == "Capsule")
                    {
                        far = far - 1;
                        if (far <= 0)
                        {
    ...
    public void gloanteGenerator()
    {
        ...
        instanceBullet1 = Instantiate(prefabBullet, transform.Find("enn_shoot_dummie").transform.position, Quaternion.LookRotation(dirLookEgo)) as GameObject;
        shootForce = 1000;
        instanceBullet1.GetComponent<Rigidbody>().mass = 5;
        instanceBullet1.GetComponent<Rigidbody>().AddForce(dirLookEgo * shootForce);
        Destroy(instanceBullet1, 1.0f);

        // Destroy(instanceSound, 1.0f);
        Destroy(instanceBullet1, 2.5f);
    }
    ...
    public void roteste_corpul_spre_ego()
    {
       ...
       //mesh_agent.transform.rotation = Quaternion.RotateTowards(mesh_agent.transform.rotation, Quaternion.LookRotation(dirLookEgo), 10.0f);//cu cat mai mare -> mai rapid


        //Vector3 targetDir = target.position - transform.position;
        //float step = speed * Time.deltaTime;
        Vector3 newDir = Vector3.RotateTowards(mesh_agent.transform.forward, egos.transform.position - mesh_agent.transform.position, 0.3f, 0.0F);
    ...
    void Update()
    {
        ...
        if (inert == 0)//CAT TIMP ESTE ENN-UL VIU
        {
            ...
            if (Physics.Raycast(transform.Find("enn_shoot_dummie").transform.position, dirLookEgo, out recFw, 200.0f))
            {
                if (recFw.collider)
                {
                    if (recFw.collider.name == "Capsule")
                    {
            ...
            if (contact_visual == 1 && distance <= 50.0f)
            {
                aproape = 1;
            }
            if (contact_visual == 0 || distance > 80.0f)
            {
