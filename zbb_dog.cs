...
public class zbb_dog : MonoBehaviour
{
    ...
    public animations_dog_0 movings;
    ...
    void Start()
    {
        ...
        */
        ...
        resp = new List<float>();
        atack = 2;
        movings = mesh_agent.GetComponent<animations_dog_0>();
    ...
    public void inert_agent_enabled_false()//miscarea initiala, fara miscare, din care trece in patrol
    {
        if (movement == "inert_agent_enabled_false")
        {
            ...
            if (once_start == 0)
            {
                ...
                no_clones_time = 10 * (snts.enns_number+1);
                once_start = 1;
            }
            ...
            if (snts.proxi == 0)
            {
              ...
              // {
                ...
                */
                ...
                if (reg_distance > 4.0f)
                {
                    setAction = setAction + 1;
                    if (setAction <= 1 * no_clones_time)
                    {
    ...
    public void roteste_corpul_spre_ego()
    {
       ...
       if(mesh_agent)
      mesh_agent.transform.rotation = Quaternion.RotateTowards(mesh_agent.transform.rotation, Quaternion.LookRotation(dirLookEgo), 6.0f);//cu cat mai mare -> mai rapid
    ...
    public void roteste_corpul_spre_inainte()
    {
        ...
        if(mesh_agent)
       mesh_agent.transform.rotation = Quaternion.RotateTowards(mesh_agent.transform.rotation, Quaternion.LookRotation(transform.forward), 4.0f);//cu cat mai mare -> mai rapid
